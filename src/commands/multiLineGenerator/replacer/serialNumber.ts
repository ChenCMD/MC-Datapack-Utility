...
export const serialNumberReplacer: Replacer = async (insertString, insertCount) => {
    const radix = parseInt(await listenInput(locale('serial-number.radix'), v => numberValidator(v, 10, { min: 2, max: 36 }), 10));
    const start = parseInt(await listenInput(locale('serial-number.start'), v => numberValidator(v, radix), 0), radix);
    const { extend: operator } = await listenPickItem(locale('serial-number.operator'), makeExtendQuickPickItem(operatorMap), false);
    const step = parseInt(await listenInput(locale('serial-number.step'), v => numberValidator(v, radix, { min: 1 }), 1), radix);
